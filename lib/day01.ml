...
module Day01 : Solution = struct
  type rotation = Left of int | Right of int
  type t = rotation list
  ...
  let parse_rotation =
    choice
      [
        (char 'L' *> parse_value >>| fun x -> Left x);
        (char 'R' *> parse_value >>| fun x -> Right x);
      ]

  let parse_list = many parse_rotation

  let parse_input str =
    match parse_string ~consume:All parse_list str with
    | Ok v -> Ok v
    | Error msg ->
  ...
  let rotate rotation position =
    match rotation with
    | Left value ->
        if position < value then (100 - ((value - position) mod 100)) mod 100
        else (position - value) mod 100
    | Right value -> (position + value) mod 100

  let compute_simple rotation_list =
    let zero_count, _ =
      List.fold_left
        (fun (zero_count, position) rotation ->
          ( (zero_count + if rotate rotation position = 0 then 1 else 0),
  ...
  let rec unroll_rotation = function
    | Left value when value <= 0 -> []
    | Right value when value <= 0 -> []
    | Left value -> Left 1 :: unroll_rotation (Left (value - 1))
    | Right value -> Right 1 :: unroll_rotation (Right (value - 1))

  let compute_advanced rotation_list =
    let zero_count, _ =
      List.concat_map unroll_rotation rotation_list
      |> List.fold_left
           (fun (zero_count, position) rotation ->
             ( (zero_count + if rotate rotation position = 0 then 1 else 0),
               rotate rotation position ))
