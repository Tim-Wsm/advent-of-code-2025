...
module Day02 : Solution = struct
  type range_t = Range of int * int
  type t = range_t list
  ...
  let parse_input str =
    match parse_string ~consume:Prefix (sep_by1 (char ',') parse_range) str with
    | Ok v -> Ok v
    | Error msg ->
  ...
  (*--------------------------------------------------------------------------*)
  let rec filter_range f = function
    | Range (r_start, r_end) when r_start = r_end ->
        if f r_start then [ r_start ] else []
    | Range (r_start, r_end) ->
        if f r_start then r_start :: filter_range f (Range (r_start + 1, r_end))
        else filter_range f (Range (r_start + 1, r_end))

  let invalid_id_simple id =
    let id_str = string_of_int id in
    let id_len = String.length id_str in
    if id_len mod 2 <> 0 then false
    else
      String.sub id_str 0 (id_len / 2)
      = String.sub id_str (id_len / 2) (id_len / 2)

  let compute_simple ranges =
    let sum =
      ranges
      |> List.concat_map (fun range -> filter_range invalid_id_simple range)
  ...
  let rec gen_sub_ids id_str sub_len =
    if id_str = "" then []
    else
      let head = String.sub id_str 0 sub_len in
      let rest = String.sub id_str sub_len (String.length id_str - sub_len) in
      head :: gen_sub_ids rest sub_len

  let rec valid_splits acc id_len =
    if acc = 0 then []
    else if id_len mod acc = 0 then acc :: valid_splits (acc - 1) id_len
    else valid_splits (acc - 1) id_len

  let invalid_id_advanced id =
    let id_str = string_of_int id in
    let id_len = String.length id_str in
    valid_splits (id_len - 1) id_len
    |> List.exists (fun split_len ->
        let sub_ids = gen_sub_ids id_str split_len in
        List.for_all
          (fun id_1 -> List.for_all (fun id_2 -> id_1 = id_2) sub_ids)
          sub_ids)

  let compute_advanced ranges =
    let sum =
      ranges
      |> List.concat_map (fun range -> filter_range invalid_id_advanced range)
      |> List.fold_left (fun acc value -> acc + value) 0
    in
